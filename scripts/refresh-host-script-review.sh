...
assert_verification_summary_artifact_input_file_class_counts() {
    local root="$1"
    python3 - "$root" <<'PY'
import json
import sys
...
summary = json.loads((root / "verification-summary.json").read_text(encoding="utf-8"))
artifact_inputs = summary.get("artifact_inputs") or {}
counts = {"json": 0, "html": 0, "bmp": 0, "other": 0}
for name in artifact_inputs:
    ...
    if suffix == ".json":
        ...
        counts["other"] += 1
if summary.get("artifact_input_file_class_counts") != counts:
    raise SystemExit("verification-summary artifact_input_file_class_counts mismatch")
print(
    "verification-summary artifact input file class counts: ok "
    f"json={counts['json']} html={counts['html']} bmp={counts['bmp']} other={counts['other']}"
)
PY
}

assert_verification_summary_artifact_input_depth_counts() {
    local root="$1"
    python3 - "$root" <<'PY'
import json
import sys
...
if summary.get("artifact_input_depth_counts") != counts:
    raise SystemExit("verification-summary artifact_input_depth_counts mismatch")
print(
    "verification-summary artifact input depth counts: ok "
    + ",".join(f"{depth}:{counts[depth]}" for depth in sorted(counts, key=int))
)
PY
}

assert_verification_summary_artifact_input_max_depth() {
    local root="$1"
    python3 - "$root" <<'PY'
import json
import sys
...
summary = json.loads((root / "verification-summary.json").read_text(encoding="utf-8"))
artifact_inputs = summary.get("artifact_inputs") or {}
actual = max((0 if name == "." else name.count("/") + 1) for name in artifact_inputs) if artifact_inputs else 0
if int(summary.get("artifact_input_max_depth", -1)) != actual:
    raise SystemExit("verification-summary artifact_input_max_depth mismatch")
print(f"verification-summary artifact input max depth: ok depth={actual}")
PY
}

assert_verification_summary_artifact_input_min_nonroot_depth() {
    local root="$1"
    python3 - "$root" <<'PY'
import json
import sys
...
summary = json.loads((root / "verification-summary.json").read_text(encoding="utf-8"))
artifact_inputs = summary.get("artifact_inputs") or {}
depths = [name.count("/") + 1 for name in artifact_inputs if name != "."]
actual = min(depths) if depths else 0
if int(summary.get("artifact_input_min_nonroot_depth", -1)) != actual:
    raise SystemExit("verification-summary artifact_input_min_nonroot_depth mismatch")
print(f"verification-summary artifact input min nonroot depth: ok depth={actual}")
PY
}

assert_verification_summary_artifact_input_parent_dirs_sha256() {
    local root="$1"
    python3 - "$root" <<'PY'
import json
import hashlib
...
summary = json.loads((root / "verification-summary.json").read_text(encoding="utf-8"))
artifact_inputs = summary.get("artifact_inputs") or {}
parent_dirs = sorted({str(Path(name).parent) for name in artifact_inputs})
expected = hashlib.sha256("\n".join(parent_dirs).encode("utf-8")).hexdigest()
actual = summary.get("artifact_input_parent_dirs_sha256")
if actual != expected:
    raise SystemExit("verification-summary artifact_input_parent_dirs_sha256 mismatch")
print(f"verification-summary artifact input parent dirs sha256: ok {expected}")
PY
}

assert_verification_summary_artifact_input_parent_dir_count() {
    local root="$1"
    python3 - "$root" <<'PY'
import json
import sys
...
summary = json.loads((root / "verification-summary.json").read_text(encoding="utf-8"))
artifact_inputs = summary.get("artifact_inputs") or {}
actual = len({str(Path(name).parent) for name in artifact_inputs})
if int(summary.get("artifact_input_parent_dir_count", -1)) != actual:
    raise SystemExit("verification-summary artifact_input_parent_dir_count mismatch")
print(f"verification-summary artifact input parent dir count: ok count={actual}")
PY
}

assert_verification_summary_artifact_input_parent_dir_depth_counts() {
    local root="$1"
    python3 - "$root" <<'PY'
import json
import sys
...
summary = json.loads((root / "verification-summary.json").read_text(encoding="utf-8"))
artifact_inputs = summary.get("artifact_inputs") or {}
parent_dirs = {str(__import__("pathlib").Path(name).parent) for name in artifact_inputs}
counts = {}
for relpath in parent_dirs:
    depth = 0 if relpath == "." else relpath.count("/") + 1
    counts[str(depth)] = counts.get(str(depth), 0) + 1
counts = dict(sorted(counts.items(), key=lambda item: int(item[0])))
if summary.get("artifact_input_parent_dir_depth_counts") != counts:
    raise SystemExit("verification-summary artifact_input_parent_dir_depth_counts mismatch")
print(
    "verification-summary artifact input parent dir depth counts: ok "
    + ",".join(f"{depth}:{counts[depth]}" for depth in sorted(counts, key=int))
)
PY
}

assert_verification_summary_artifact_input_parent_dir_max_depth() {
    local root="$1"
    python3 - "$root" <<'PY'
import json
import sys
...
summary = json.loads((root / "verification-summary.json").read_text(encoding="utf-8"))
artifact_inputs = summary.get("artifact_inputs") or {}
parent_dirs = {str(Path(name).parent) for name in artifact_inputs}
actual = max((0 if relpath == "." else relpath.count("/") + 1) for relpath in parent_dirs) if parent_dirs else 0
if int(summary.get("artifact_input_parent_dir_max_depth", -1)) != actual:
    raise SystemExit("verification-summary artifact_input_parent_dir_max_depth mismatch")
print(f"verification-summary artifact input parent dir max depth: ok depth={actual}")
PY
}

assert_verification_summary_artifact_input_parent_dir_min_nonroot_depth() {
    local root="$1"
    python3 - "$root" <<'PY'
import json
import sys
...
artifact_inputs = summary.get("artifact_inputs") or {}
parent_dirs = {str(Path(name).parent) for name in artifact_inputs}
depths = [relpath.count("/") + 1 for relpath in parent_dirs if relpath != "."]
actual = min(depths) if depths else 0
if int(summary.get("artifact_input_parent_dir_min_nonroot_depth", -1)) != actual:
    raise SystemExit("verification-summary artifact_input_parent_dir_min_nonroot_depth mismatch")
print(f"verification-summary artifact input parent dir min nonroot depth: ok depth={actual}")
PY
}

assert_verification_summary_artifact_input_parent_dir_basenames_sha256() {
    local root="$1"
    python3 - "$root" <<'PY'
import json
import hashlib
...
summary = json.loads((root / "verification-summary.json").read_text(encoding="utf-8"))
artifact_inputs = summary.get("artifact_inputs") or {}
parent_dir_basenames = sorted({Path(name).parent.name for name in artifact_inputs})
expected = hashlib.sha256("\n".join(parent_dir_basenames).encode("utf-8")).hexdigest()
actual = summary.get("artifact_input_parent_dir_basenames_sha256")
if actual != expected:
    raise SystemExit("verification-summary artifact_input_parent_dir_basenames_sha256 mismatch")
print(f"verification-summary artifact input parent dir basenames sha256: ok {expected}")
PY
}

assert_verification_summary_artifact_input_parent_dir_basename_count() {
    local root="$1"
    python3 - "$root" <<'PY'
import json
import sys
...
summary = json.loads((root / "verification-summary.json").read_text(encoding="utf-8"))
artifact_inputs = summary.get("artifact_inputs") or {}
actual = len({Path(name).parent.name for name in artifact_inputs})
if int(summary.get("artifact_input_parent_dir_basename_count", -1)) != actual:
    raise SystemExit("verification-summary artifact_input_parent_dir_basename_count mismatch")
print(f"verification-summary artifact input parent dir basename count: ok count={actual}")
PY
}
...
expected = (
    ...
    f"{classes.get('json', 0)}j/{classes.get('html', 0)}h/{classes.get('bmp', 0)}b/{classes.get('other', 0)}o|"
    f"{summary.get('artifact_input_max_depth')}|"
    f"{summary.get('artifact_input_min_nonroot_depth')}|"
    f"{summary.get('artifact_input_parent_dir_count')}|"
    f"{summary.get('artifact_input_parent_dir_max_depth')}|"
    f"{summary.get('artifact_input_parent_dir_min_nonroot_depth')}|"
    f"{summary.get('artifact_input_parent_dir_basename_count')}"
)
if summary.get("artifact_input_contract") != expected:
...
if summary.get("artifact_input_contract_sha256") != expected_sha256:
...
review_paths = {
    "index_html": rel_to_root(root / "index.html"),
    "identification_review_html": rel_to_root(root / "identification-review.html"),
    "capture_regression_review_html": rel_to_root(root / "capture-regression-review.html"),
}
core_artifact_paths = {
    "manifest_json": rel_to_root(root / "manifest.json"),
    "semantic_truth_json": rel_to_root(root / "semantic-truth.json"),
}
identification_audit_paths = {
    "selfcheck_json": rel_to_root(root / "identification-selfcheck.json"),
    "eval_json": rel_to_root(root / "identification-eval.json"),
    "partials_json": rel_to_root(root / "identification-partials.json"),
    "challenges_json": rel_to_root(root / "identification-challenges.json"),
    "temporal_json": rel_to_root(root / "identification-temporal.json"),
}
identification_floor_paths = {
    "regression_floors_json": rel_to_root(root / "identification-regression-floors.json"),
}
host_truth_paths = {
    "baseline_json": rel_to_root(root / "host-truth-baseline.json"),
    "compare_json": rel_to_root(root / "host-truth-compare.json"),
    "compare_html": rel_to_root(root / "host-truth-compare.html"),
}
expectation_paths = {
    "baseline_json": rel_to_root(root / "expectations.json"),
    "report_json": rel_to_root(root / "expectation-report.json"),
    "report_html": rel_to_root(root / "expectation-report.html"),
}
repro_paths = {
    "compare_json": rel_to_root(root / "repro-compare.json"),
    "compare_html": rel_to_root(root / "repro-compare.html"),
}
capture_audit_paths = {
    "image_report_json": rel_to_root(root / "frame-image-regression-report.json"),
    "meta_report_json": rel_to_root(root / "frame-meta-regression-report.json"),
    "semantic_report_json": rel_to_root(root / "semantic-regression-report.json"),
    "capture_report_json": rel_to_root(root / "capture-regression-report.json"),
}
regression_baseline_paths = {
    "image_baseline_json": rel_to_root(root / "frame-image-regression-baseline.json"),
    "meta_baseline_json": rel_to_root(root / "frame-meta-regression-baseline.json"),
    "semantic_baseline_json": rel_to_root(root / "semantic-regression-baseline.json"),
}
scene_root_paths = {
    "fishing_scene_dir": rel_to_root(root / "fishing1"),
    "mary_scene_dir": rel_to_root(root / "mary1"),
}
scene_asset_paths = {
    "fishing_frames_dir": rel_to_root(root / "fishing1" / "frames"),
    "fishing_meta_dir": rel_to_root(root / "fishing1" / "frame-meta"),
    "mary_frames_dir": rel_to_root(root / "mary1" / "frames"),
    "mary_meta_dir": rel_to_root(root / "mary1" / "frame-meta"),
}
key_frame_paths = {
    "fishing_start_bmp": rel_to_root(root / "fishing1" / "frames" / f"{fishing_start_name}.bmp"),
    "fishing_late_bmp": rel_to_root(root / "fishing1" / "frames" / f"{fishing_late_name}.bmp"),
    "mary_start_bmp": rel_to_root(root / "mary1" / "frames" / f"{mary_start_name}.bmp"),
    "mary_late_bmp": rel_to_root(root / "mary1" / "frames" / f"{mary_late_name}.bmp"),
}
...
key_frame_meta_paths = {
    "fishing_start_json": rel_to_root(resolve_key_frame_meta("fishing1", fishing_start_name)),
    "fishing_late_json": rel_to_root(resolve_key_frame_meta("fishing1", fishing_late_name)),
    "mary_start_json": rel_to_root(resolve_key_frame_meta("mary1", mary_start_name)),
    "mary_late_json": rel_to_root(resolve_key_frame_meta("mary1", mary_late_name)),
}
...
summary["artifact_input_file_class_counts"] = {
    "json": sum(1 for name in digest_inputs if Path(name).suffix.lower() == ".json"),
    "html": sum(1 for name in digest_inputs if Path(name).suffix.lower() == ".html"),
    "bmp": sum(1 for name in digest_inputs if Path(name).suffix.lower() == ".bmp"),
    "other": sum(
        1
        for name in digest_inputs
        if Path(name).suffix.lower() not in {".json", ".html", ".bmp"}
    ),
}
artifact_input_depth_counts = {}
for name in digest_inputs:
    depth = 0 if name == "." else name.count("/") + 1
    artifact_input_depth_counts[str(depth)] = artifact_input_depth_counts.get(str(depth), 0) + 1
summary["artifact_input_depth_counts"] = dict(
    sorted(artifact_input_depth_counts.items(), key=lambda item: int(item[0]))
)
summary["artifact_input_max_depth"] = max(
    (0 if name == "." else name.count("/") + 1) for name in digest_inputs
) if digest_inputs else 0
artifact_input_nonroot_depths = [name.count("/") + 1 for name in digest_inputs if name != "."]
summary["artifact_input_min_nonroot_depth"] = (
    min(artifact_input_nonroot_depths) if artifact_input_nonroot_depths else 0
)
summary["artifact_input_parent_dirs_sha256"] = hashlib.sha256(
    "\n".join(sorted({str(Path(name).parent) for name in digest_inputs})).encode("utf-8")
).hexdigest()
summary["artifact_input_parent_dir_count"] = len({str(Path(name).parent) for name in digest_inputs})
artifact_input_parent_dir_depth_counts = {}
for relpath in {str(Path(name).parent) for name in digest_inputs}:
    depth = 0 if relpath == "." else relpath.count("/") + 1
    artifact_input_parent_dir_depth_counts[str(depth)] = (
        artifact_input_parent_dir_depth_counts.get(str(depth), 0) + 1
    )
summary["artifact_input_parent_dir_depth_counts"] = dict(
    sorted(artifact_input_parent_dir_depth_counts.items(), key=lambda item: int(item[0]))
)
parent_dir_relpaths = {str(Path(name).parent) for name in digest_inputs}
summary["artifact_input_parent_dir_max_depth"] = max(
    (0 if relpath == "." else relpath.count("/") + 1) for relpath in parent_dir_relpaths
) if parent_dir_relpaths else 0
parent_dir_nonroot_depths = [relpath.count("/") + 1 for relpath in parent_dir_relpaths if relpath != "."]
summary["artifact_input_parent_dir_min_nonroot_depth"] = (
    min(parent_dir_nonroot_depths) if parent_dir_nonroot_depths else 0
)
summary["artifact_input_parent_dir_basenames_sha256"] = hashlib.sha256(
...
summary["artifact_input_contract_sha256"] = hashlib.sha256(
    summary["artifact_input_contract"].encode("utf-8")
).hexdigest()
(
...
(root / "verification-summary.txt").write_text(
    ...
    "expectation-report={expectation} host-truth-compare={host_truth} repro-compare={repro}\n".format(
        ...
        artifact_input_file_class_counts=(
            f"{summary['artifact_input_file_class_counts']['json']}j/"
            f"{summary['artifact_input_file_class_counts']['html']}h/"
            f"{summary['artifact_input_file_class_counts']['bmp']}b/"
            f"{summary['artifact_input_file_class_counts']['other']}o"
        ),
        artifact_input_depth_counts=",".join(
            f"{depth}:{summary['artifact_input_depth_counts'][depth]}"
            for depth in sorted(summary["artifact_input_depth_counts"], key=int)
        ),
        artifact_input_max_depth=summary["artifact_input_max_depth"],
        artifact_input_min_nonroot_depth=summary["artifact_input_min_nonroot_depth"],
        artifact_input_parent_dirs_sha256=summary["artifact_input_parent_dirs_sha256"],
        artifact_input_parent_dir_count=summary["artifact_input_parent_dir_count"],
        artifact_input_parent_dir_depth_counts=",".join(
            f"{depth}:{summary['artifact_input_parent_dir_depth_counts'][depth]}"
            for depth in sorted(summary["artifact_input_parent_dir_depth_counts"], key=int)
        ),
        artifact_input_parent_dir_max_depth=summary["artifact_input_parent_dir_max_depth"],
...
assert_verification_summary_artifact_input_file_class_counts "$OUT_DIR"
assert_verification_summary_artifact_input_depth_counts "$OUT_DIR"
assert_verification_summary_artifact_input_max_depth "$OUT_DIR"
assert_verification_summary_artifact_input_min_nonroot_depth "$OUT_DIR"
assert_verification_summary_artifact_input_parent_dirs_sha256 "$OUT_DIR"
assert_verification_summary_artifact_input_parent_dir_count "$OUT_DIR"
assert_verification_summary_artifact_input_parent_dir_depth_counts "$OUT_DIR"
assert_verification_summary_artifact_input_parent_dir_max_depth "$OUT_DIR"
...
assert_verification_summary_artifact_input_contract "$OUT_DIR"

...
if command -v docker &> /dev/null; then
    ...
    echo "✗ Docker NOT installed - run: ./setup-docker.sh"
fi
echo ""
...
if docker images | grep -q "jc-reborn-ps1-dev"; then
    ...
    echo "✗ PS1 development image NOT found - run: ./build-docker-image.sh"
fi
echo ""
...
if [ -f "build-ps1/jcreborn.elf" ]; then
    ...
    echo "✗ jcreborn.elf NOT found - run: ./build-ps1.sh"
fi
...
if [ -f "build-ps1/jcreborn.exe" ]; then
    ...
    echo "✗ jcreborn.exe NOT found - run: ./build-ps1.sh"
fi
...
if [ -f "jcreborn.cue" ] && [ -f "jcreborn.bin" ]; then
    ...
    echo "✗ CD image NOT found - run: ./make-cd-image.sh"
fi
echo ""
...
if [ -f "build-ps1/jcreborn.elf" ] && docker images | grep -q "jc-reborn-ps1-dev"; then
    echo "=== Build Analysis ==="
    ./analyze-build.sh build-ps1/jcreborn.elf 2>/dev/null || echo "Unable to run analysis"
    echo ""
fi
...
if ! command -v docker &> /dev/null; then
    echo "⚠️  Install Docker: ./setup-docker.sh"
    READY=false
fi

if ! docker images | grep -q "jc-reborn-ps1-dev"; then
    echo "⚠️  Build Docker image: ./build-docker-image.sh"
    READY=false
fi

if [ ! -f "build-ps1/jcreborn.exe" ]; then
    echo "⚠️  Build executable: ./build-ps1.sh"
    READY=false
fi

if [ ! -f "jcreborn.cue" ]; then
    echo "⚠️  Create CD image: ./make-cd-image.sh"
    READY=false
fi
...
if [ "$READY" = true ]; then
    ...
    echo ""
    echo "Next steps:"
    echo "  ./test-ps1.sh         - Launch in DuckStation"
    echo "  ./auto-test-ps1.sh    - Automated test with screenshot"
else
    echo ""

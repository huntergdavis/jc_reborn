...
if [ ! -f "config/ps1/Dockerfile.ps1" ]; then
    echo "ERROR: config/ps1/Dockerfile.ps1 not found!"
    exit 1
fi
...
docker build --platform linux/amd64 -f config/ps1/Dockerfile.ps1 -t jc-reborn-ps1-dev:amd64 .
...
echo ""
echo "Next steps:"
echo "  ./scripts/build-ps1.sh        - Build the PS1 executable"
echo "  ./scripts/make-cd-image.sh    - Create CD image"
echo "  ./scripts/test-ps1.sh         - Test in DuckStation"
echo ""
